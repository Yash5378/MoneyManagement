...
const useApp = () => {
  ...
  const [hasReceiveSMSPermission, setHasReceiveSMSPermission] = useState<
    boolean | null
  >(null);
  const [hasReadSMSPermission, setHasReadSMSPermission] = useState<
    boolean | null
  >(null);
  const [smsPermissionState, setSmsPermissionState] = useState<string | null>(
    null
  );
  const [successCallbackStatus, setSuccessCallbackStatus] = useState<
    string | null
  >(null);
  const [errorCallbackStatus, setErrorCallbackStatus] = useState<string | null>(
    null
  );
